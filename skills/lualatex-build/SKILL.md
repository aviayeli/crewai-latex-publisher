...
## CRITICAL: `templates/preamble.tex` Is READ-ONLY — IMMUTABLE Outside STEP 0

**`templates/preamble.tex` MUST NOT be modified, overwritten, or regenerated by the Compiler Agent at any point other than STEP 0 (the explicit audit-and-repair step above).**

This file is the single source of truth for the entire build pipeline. Any modification outside of STEP 0 introduces non-determinism: different articles will compile against different preamble states, headers and footers will drift, and font/package configuration will become unpredictable.

**FORBIDDEN actions (outside STEP 0):**
- Writing or overwriting `templates/preamble.tex` with `latex_writer_tool`.
- Injecting any `\usepackage`, `\pagestyle`, `\fancyhdr`, or `\addbibresource` directive into `preamble.tex` to "fix" a per-article compilation error.
- Appending or prepending content to `preamble.tex` as a workaround.

**REQUIRED behavior:**
- Treat `templates/preamble.tex` as an immutable constant after STEP 0 completes.
- If a compilation error references the preamble, diagnose the error in the article's own `.tex` files or `refs.bib`, not by touching `preamble.tex`.
- If you believe `preamble.tex` must be updated, escalate to the Manager Agent with `[PREAMBLE CHANGE REQUEST]` and halt — do not modify it unilaterally.
