...
## ABSOLUTE PROHIBITION: DO NOT WRITE OR MODIFY `main.tex`

**You MUST NOT write, overwrite, or append to `latex_output/main.tex` under any circumstances.**

`main.tex` is assembled exclusively by the `compiler_agent` (LaTeX Build Engineer) from
`templates/preamble.tex` — the single source of truth for all packages, fonts, fancyhdr
layout, and page setup. The outline agent has no role in that assembly.

**FORBIDDEN actions (contract violation if violated):**
- Writing any content to `latex_output/main.tex`
- Injecting `\AtBeginDocument`, `\AfterEndPreamble`, `\fancyhdr`, `\pagestyle`, or any
  layout macro into any file other than chapter files
- Appending preamble fragments, TOC macros (`\l@chapter`, `\@dottedtocline`), or font
  declarations to `main.tex`

**Your ONLY output artifact is `latex_output/book_outline.json`.**

Emit this checkpoint when ready to proceed: `[CHECKPOINT] STEP 0 skipped — outline agent does not touch main.tex. Proceeding to book_outline.json.`
